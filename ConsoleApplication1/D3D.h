...
{
	class D3DXVECTOR3                                     Location;                                                 // 0x0000(0x000C) (CPF_Edit, CPF_BlueprintVisible, CPF_ZeroConstructor, CPF_IsPlainOldData)
	class D3DXVECTOR3                                      Rotation;                                                 // 0x000C(0x000C) (CPF_Edit, CPF_BlueprintVisible, CPF_ZeroConstructor, CPF_IsPlainOldData)
...
namespace D // Global Stuff
{
	...
	static list<int> upper_part = { Bones::neck_01, Bones::Head, Bones::forehead };
	static list<int> right_arm = { Bones::neck_01, Bones::upperarm_r, Bones::lowerarm_r, Bones::hand_r };
	static list<int> left_arm = { Bones::neck_01, Bones::upperarm_l, Bones::lowerarm_l, Bones::hand_l };
	static list<int> spine = { Bones::neck_01, Bones::spine_01, Bones::spine_02, Bones::pelvis };

	static list<int> lower_right = { Bones::pelvis, Bones::thigh_r, Bones::calf_r, Bones::foot_r };
	static list<int> lower_left = { Bones::pelvis, Bones::thigh_l, Bones::calf_l, Bones::foot_l };

	static list<list<int>> skeleton = { upper_part, right_arm, left_arm, spine, lower_right, lower_left };
	static int  cx;
	static int  cy;
	static IDirect3D9* p_Object = 0;
	static IDirect3DDevice9* p_Device = 0;
	static D3DPRESENT_PARAMETERS p_Params;

	static ID3DXLine* p_Line;
	static ID3DXFont* pFontSmall = 0;
	static ID3DXFont* pFontBigGBK = 0;
	static ID3DXFont* pu8Font = 0;
	static ID3DXFont* pFontBig = 0;
	static ID3DXFont* pFontBigUTF = 0;
	static ID3DXFont* pFontSuperBig = 0;
	static ID3DXFont* pFontSuperBigUTF = 0;

	static int Width = 0;
	static int Height = 0;
	static char lWindowName[256];
	static HWND hWnd;
	static char tWindowName[256];
	static HWND tWnd;
	static RECT tSize;
	static MSG Message;
	static bool Debug_Border;
	static const MARGINS Margin = { 0, 0, Width, Height };
//	static HWND WindowsHandle;
	...
	void ExitHax();
	void GetAxes(D3DXVECTOR3 Rot, D3DXVECTOR3 & X, D3DXVECTOR3 & Y, D3DXVECTOR3 & Z);
	bool toScreen(D3DXVECTOR3 rotat, D3DXVECTOR3 location, float fov, D3DXVECTOR3 in, D3DXVECTOR2 & screenPosition);
	void clearscnz();
	void presentz();
	void zRender();
	void AimBot();
	void Render();
	void DrawLines(D3DXVECTOR2 point0[5]);
	void Draw3DBox(D3DXVECTOR3 rotat, D3DXVECTOR3 location, float fov, bool isCar, D3DXVECTOR3 loc, D3DXVECTOR3 rot);
	int Start();
	...
	int WDstr(wchar_t * zString, int x, int y, int r, int g, int b, ID3DXFont * ifont);
	void DrawBox(float x, float y, float width, float height, D3DCOLOR color);
	void DrawBox(float x, float y, float width, float height, D3DCOLOR color, int wdt);
	FTransform GetBoneIndex(DWORD_PTR mesh, int index);
	D3DMATRIX MatrixMultiplication(D3DMATRIX pM1, D3DMATRIX pM2);
	D3DXVECTOR3 GetBoneWithRotation(DWORD_PTR mesh, int id);
	void DrawSkeleton(D3DXVECTOR3 rotat, D3DXVECTOR3 location, float fov, DWORD_PTR mesh, float color);
	...
	int DrawString(char * String, int x, int y, int r, int g, int b, ID3DXFont * ifont, bool isCenter);
}