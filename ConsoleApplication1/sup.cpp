...
)
{
  ...
  RtlInitUnicodeStringEX fpRtlInitUnicodeString =
	  (RtlInitUnicodeStringEX)GetProcAddress(GetModuleHandle(_T("ntdll")), "RtlInitUnicodeString");

  fpRtlInitUnicodeString(&NtFileName, (LPWSTR)FilePath);
  InitializeObjectAttributes(&ObjAttr, &NtFileName, 0, NULL, NULL);
  static NtCreateFileEx fpNtCreateFile =
	  (NtCreateFileEx)GetProcAddress(GetModuleHandle(_T("ntdll")), "NtCreateFile");

  Status = fpNtCreateFile(
    &FileHandle,
    SYNCHRONIZE,
    &ObjAttr,
    &IoStatus,
  ...
  NTCLOSE fpNtClose =
	  (NTCLOSE)GetProcAddress(GetModuleHandle(_T("ntdll")), "NtClose");
...
)
{
  ...
  status = fpNtQuerySystemInformation(
